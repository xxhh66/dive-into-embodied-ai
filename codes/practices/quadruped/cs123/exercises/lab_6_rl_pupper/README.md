...
教程 §6.7 / §6.8 各跑了一次"暴走"和"优雅"奖励对比，但**没把训练真正跑完 2M 步、没保存 checkpoint、没录变速命令 demo**。这就是 Lab 6 的增量：在 CPU 上用 `stable-baselines3` PPO 端到端训出一个能跟随变速命令的策略，拿到你自己的 `pupper_ppo.zip`。
...
| 候选 | 教程覆盖 | 门槛 | 作品价值 |
|---|---|---|---|
| Reward tuning 遍历 | §6.4–§6.8 已覆盖 | 低 | 中（图表） |
| Domain randomization | §6.5 | 中 | 中 |
...
1. **环境拼接（30 min）**：读懂 `PupperEnv.__init__` 里的 PD 残差设定，补 `_get_obs`。（对应教程 §6.2）
2. **奖励设计（30 min）**：补 `_compute_reward`，理解为什么 `r_torque` 系数是 2e-4。（对应教程 §6.4）
3. **命令分布（10 min）**：补 `_sample_command`，选合理范围。（对应教程 §6.2）
4. **训练（30–60 min）**：`uv run python lab_6_rl_pupper/train_ppo.py`，看 tensorboard。（对应教程 §6.6）
5. **评估录屏（20 min）**：`uv run python lab_6_rl_pupper/eval_commands.py`。（对应教程 §6.9）
...
- **复用**：教程 §6.2 环境封装、§6.3 PD 残差、§6.4 奖励配置、§6.6 PPO 训练脚本、§6.9 评估结构。
- **扩展**：把"训练一个会走的策略"完整跑完一次；第一次获得可被 Lab 7 调用的 checkpoint。
- **不重复**：教程 §6.7 暴走 vs §6.8 优雅对比作为前置练习。

## 不做什么

- 不做 §6.5 域随机化（Stretch 留给学生自己加）
- 不做完整稳定性 / 接触 / 地形组奖励
- 不做 MJX / JAX / Brax
- 不做 sim2real / 真机 / 真相机
