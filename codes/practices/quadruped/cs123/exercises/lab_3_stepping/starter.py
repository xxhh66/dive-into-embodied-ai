"""Lab 3 filled starter：Pupper 单腿 Raibert 悬空踏步。
...
def raibert_foot_traj(
    ...
    *,
    ...
    if phase.in_stance:
        ...
        s = phase.local_phase
        x = -half_step + step_length * s
        lift = 4.0 * s * (1.0 - s) * step_height * mid_swing_lift
        z = z_stance + lift
    return np.array((x, FOOT_Y, z), dtype=float)
...
def dls_ik_step(
    ...
    *,
    ...
    ctx.data.qpos[ctx.qpos_ids] = qpos
    mujoco.mj_forward(ctx.model, ctx.data)
    current = ctx.data.site_xpos[ctx.foot_id].copy()
    err = pose_error(target_xyz, current)

    jacp = np.zeros((3, ctx.model.nv), dtype=float)
    mujoco.mj_jacSite(ctx.model, ctx.data, jacp, None, ctx.foot_id)
    J = jacp[:, ctx.qvel_ids]
    try:
        dtheta = damped_pinv(J, lam) @ err
    except np.linalg.LinAlgError:
        dtheta = np.linalg.pinv(J, rcond=1e-12) @ err

    q_next = np.clip(qpos + step * dtheta, JOINT_LIMITS[:, 0], JOINT_LIMITS[:, 1])
    ctx.data.qpos[ctx.qpos_ids] = q_next
    mujoco.mj_forward(ctx.model, ctx.data)
    residual = float(np.linalg.norm(pose_error(target_xyz, ctx.data.site_xpos[ctx.foot_id])))
    return q_next, residual
