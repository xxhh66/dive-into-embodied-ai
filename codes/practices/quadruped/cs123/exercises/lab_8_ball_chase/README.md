...
到 [§8](/docs/practices/quadruped/cs123/perception) 为止，RL 的"腿"（Lab 5/6）、LLM 的"脑"（Lab 7）、相机的"眼"（[§8](/docs/practices/quadruped/cs123/perception) 教程）都做好了——但从来没有让它们在同一段视频里同时工作过。Lab 8 把视觉、跟踪、RL 步态拧到一起：Pupper 看到红球、走过去、在球前停下，20 秒追球视频做"毕业汇演"。

## 为什么选这个任务

| 候选 | 教程覆盖 | 门槛 | 作品价值 |
|---|---|---|---|
| 原地转头追颜色球 | [§8.6](/docs/practices/quadruped/cs123/perception#86-原地追球) 已覆盖 | 低 | 低（只转头） |
| YOLO 检测 + 追踪 | [§8.3](/docs/practices/quadruped/cs123/perception#83-目标检测) 方案 B | 中（GPU） | 中 |
| **HSV 检测 + RL 步态追球** | ❌ | 低（纯 CPU） | **高（毕业视频）** |
| VLM 视觉闭环 | [§8.8](/docs/practices/quadruped/cs123/perception#88-接入语言-agent) Stretch | 高（API） | 高 |
...
1. **HSV 检测（30 min）**：补 `perception/ball_detector.py` 的 `detect_red_ball()`。（对应教程 [§8.3](/docs/practices/quadruped/cs123/perception#83-目标检测) 方案 A）
2. **P 控制器 + FSM（30 min）**：补 `perception/tracker_fsm.py` 的 `visual_servo()` 和 `TrackerFSM.step()`。（对应教程 [§8.4](/docs/practices/quadruped/cs123/perception#84-航向控制)–[§8.5](/docs/practices/quadruped/cs123/perception#85-跟踪状态机)）
3. **追球主循环（40 min）**：补 `starter.py` 的 `run_chase()`，理解 500 Hz physics / 50 Hz policy / 10 Hz vision 的三层频率。（对应教程 [§8.6](/docs/practices/quadruped/cs123/perception#86-原地追球)–[§8.7](/docs/practices/quadruped/cs123/perception#87-行走追球)）
...
- **复用**：教程 [§8.3](/docs/practices/quadruped/cs123/perception#83-目标检测) HSV 检测、[§8.4](/docs/practices/quadruped/cs123/perception#84-航向控制) P 控制器、[§8.5](/docs/practices/quadruped/cs123/perception#85-跟踪状态机) TrackerFSM。
- **扩展**：把"原地转头"扩展为"边走边追"，加 RL 步态闭环和 GIF 录制。
- **不重复**：教程 [§8.6](/docs/practices/quadruped/cs123/perception#86-原地追球) 原地追踪作为前置练习。

## 不做什么

- 不做 [§8.3](/docs/practices/quadruped/cs123/perception#83-目标检测) 方案 B（YOLOv8-nano，需要 GPU）
- 不做 [§8.8](/docs/practices/quadruped/cs123/perception#88-接入语言-agent) VLM/LLM 视觉接入（Stretch 留给学生）
- 不做真机 / 真相机
