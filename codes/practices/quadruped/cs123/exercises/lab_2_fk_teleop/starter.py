"""Lab 2 filled starter：Pupper 单腿 FK 与滑杆遥操作。

作者侧先写 `starter_todo.py`，再把三处算法空白填成这份 `starter.py`。
...
def fk_validate(
    *,
    ...
    max_err = 0.0
    for theta in rng.uniform(JOINT_LIMITS[:, 0], JOINT_LIMITS[:, 1], size=(n, 3)):
        data.qpos[qpos_ids] = theta
        mujoco.mj_forward(model, data)
        p_mj = data.site_xpos[foot_id].copy()
        p_fk = np.asarray(fk_fn(theta), dtype=float)
        err = float(np.linalg.norm(p_mj - p_fk))
...
def sample_workspace(n_samples: int = 20_000, *, seed: int = 0) -> np.ndarray:
    ...
    theta = rng.uniform(JOINT_LIMITS[:, 0], JOINT_LIMITS[:, 1], size=(n_samples, 3))
    return np.asarray([fk_leg(row) for row in theta], dtype=float)
