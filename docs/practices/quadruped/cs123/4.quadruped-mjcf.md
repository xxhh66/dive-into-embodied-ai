...
§4.5 让原版 Pupper 站住、§4.6 看了 5 组 `Kp/Kd` 的崩坏模式。在迈向 Ch5 让它走起来之前，本章动手任务先让你**主动改 MJCF**：把 thigh + calf 同比例拉长 1.5 倍做一只 long-leg，把 torso 质量 ×2 做一只 heavy。改完之后 §4.5 的 `stand_pose` 和 §4.6 的 `Kp/Kd` 都不再通用，你得在三只 Pupper 上各重做一遍——腿越长重心越高，身越沉甜点 `Kp` 越大。
