...
正运动学描述的是从关节角到末端位姿的映射：关节角 → 末端。逆运动学解决相反的问题：给定一个末端目标，求出对应的关节角。这一章我们用两种方式实现 IK，并把它接到 [§1](/docs/practices/quadruped/cs123/pid-control) 的 PD 控制器上，做出第一个"点到点控制" demo。
...
经验法则:**有特解就用解析，一般机械臂 / 人形 / 四足都用数值**。下面两节分别展开，[§3.5](/docs/practices/quadruped/cs123/inverse-kinematics#35-三个难题) 再回来谈三类绕不开的工程难题。
...
不论用解析还是数值，在 MuJoCo / Isaac Sim 这类仿真器或真机上写 IK，总会反复撞上同样的三类问题。**这三件事就是 [§3](/docs/practices/quadruped/cs123/inverse-kinematics) 真正的难点**——算法本身只是骨架，工程上能不能用，看的是这三个坑怎么填。
...
- **解析法显式选支**:像 [§3.3](/docs/practices/quadruped/cs123/inverse-kinematics#33-3-dof-解析法) 那样，直接用一个 `elbow_up` 布尔位钉死构型。
...
1. **DLS 阻尼**([§3.4](/docs/practices/quadruped/cs123/inverse-kinematics#34-dls-数值法) 的默认做法):用 $\lambda^2 I$ 把奇异值"垫起来"，最常用、性价比最高。
...
**不可达性(Reachability)** 指的是:目标点超出机械臂物理长度范围，数学上方程组**无解**——这正是 [§2](/docs/practices/quadruped/cs123/forward-kinematics) 那张"甜甜圈"图之所以重要的原因。严肃的 IK 实现会先做一次"目标是否可达"的保护，再进迭代。

还有一种更隐蔽的"可达但到不了":目标在工作空间里，但当前构型被关节限位、自碰撞或环境障碍物挡住，数值法只能收敛到一个局部最优。这时单跑一次 IK 注定失败，需要靠 [§3.5.1](/docs/practices/quadruped/cs123/inverse-kinematics#351-多解性) 提到的多 seed 或显式路径规划绕过去。

DLS 在这两类情形下都不会崩——它会"贴着边缘最近点"跟着目标走([§3.9](/docs/practices/quadruped/cs123/inverse-kinematics#39-viewer-看-dls-收敛) 会直观看到这一点)，这是它相对纯伪逆最大的工程优势。

## 3.6 IK + PD 控制

IK 给出的是**关节目标角**，不是力矩。把它喂进 [§1](/docs/practices/quadruped/cs123/pid-control) 那条 PD 回路，就构成了机器人学里最基础的 **Task-space → Joint-space → Torque** 三层串联：
...
[§3.4](/docs/practices/quadruped/cs123/inverse-kinematics#34-dls-数值法) 的 `ik_dls` 留了两个口子——`fk_fn` 和 `jac_fn`，这一节先把它们落地。手算雅可比对 3-DoF 平面臂还行，到 6-DoF 真机就非常容易写错；**更靠谱的工程做法是直接让 MuJoCo 替你算**：
...
`mj_jacSite` 会按当前 `qpos` 直接把 site 的几何雅可比算出来，省掉所有手推工作；要切换到不同机器人，只要换 XML 和 site 名字。**[§2](/docs/practices/quadruped/cs123/forward-kinematics) 那条"自写 FK vs `mj_forward` 误差 < 1e-12"的校验思路在这里同样适用**：写完手算的 `jac_fn` 后，跟 `jac_mj` 在同一组 $\theta$ 上比一下，能在 5 分钟内抓到 99% 的链式乘法 bug。
...
把它接到 [§3.6](/docs/practices/quadruped/cs123/inverse-kinematics#36-ik--pd-控制) 的控制循环里：
...
- **拐角越急、$K_p$ 越大跟得越紧**，但太大会震荡（[§1](/docs/practices/quadruped/cs123/pid-control) 那个老问题）。把 $K_p$ 从 40 调到 80，再调到 200，三种现象会很明显。
...
这一个实验和 [§3.7](/docs/practices/quadruped/cs123/inverse-kinematics#37-实验末端画圆) 的圆放一起，刚好覆盖了"光滑轨迹 + 不光滑轨迹"两类典型任务，已经足够 [§4](/docs/practices/quadruped/cs123/quadruped-mjcf) 四足步态或 [§5](/docs/practices/quadruped/cs123/gait-control) 末端抓取直接复用。

## 3.9 viewer 看 DLS 收敛

光看末端轨迹不够直观——把 <strong>目标点（绿球）</strong> 和 <strong>当前末端（红球）</strong> 都叠到 MuJoCo viewer 里，DLS 的"追"才能一眼看出来。这一段是 [§2](/docs/practices/quadruped/cs123/forward-kinematics) viewer 实验的直接延伸，最终叠加效果见 [图 4](#fig-ik-viewer-debug-overlay)。
...
- **三个 MuJoCo 实验串起来**：[§3.7](/docs/practices/quadruped/cs123/inverse-kinematics#37-实验末端画圆) 画圆验证光滑轨迹跟踪、[§3.8](/docs/practices/quadruped/cs123/inverse-kinematics#38-实验末端走三角形) 走三角形暴露角点处的控制带宽、[§3.9](/docs/practices/quadruped/cs123/inverse-kinematics#39-viewer-看-dls-收敛) 在 viewer 里用绿球（目标）+ 红球（当前末端）实时观察 DLS 收敛，便于排查"追不上"到底是 IK 还是 PD 的锅。这套调试范式会在 [§4](/docs/practices/quadruped/cs123/quadruped-mjcf) 四足、[§5](/docs/practices/quadruped/cs123/gait-control) 抓取里反复用到。

## 动手任务

[§3.8](/docs/practices/quadruped/cs123/inverse-kinematics#38-实验末端走三角形) 让平面臂走了个抽象三角形，[§3.9](/docs/practices/quadruped/cs123/inverse-kinematics#39-viewer-看-dls-收敛) 又把绿球（目标）和红球（当前末端）叠到了 viewer 里。本章动手任务把这两件事各往物理意义上推一格：三角形的三个顶点变成 **touchdown / standing / liftoff**，中间补一段 **mid-swing 抛物线**——这就是 **Raibert 三角**，四足所有步态的原子动作。轨迹接到 [§2](/docs/practices/quadruped/cs123/forward-kinematics) 那条 Pupper 真腿（用 `<weld>` 把 hip 挂在空中），DLS IK + [§1](/docs/practices/quadruped/cs123/pid-control) 的 PD 闭环跑起来。[§5](/docs/practices/quadruped/cs123/gait-control) 的 trot 步态就是"这条腿 × 4 + 相位偏移"，提前在这里把原子动作打磨好。
...
- [ ] Stretch：`step_length` 从 0 线性扫到 0.12 m，看"原地踏步"渐变成"D 字形迈步"——给 [§5](/docs/practices/quadruped/cs123/gait-control) trot 留个伏笔

完整 starter / 测试 / 交付清单见 [`exercises/lab_3_stepping/`](https://github.com/datawhalechina/dive-into-embodied-ai/tree/master/codes/practices/quadruped/cs123/exercises/lab_3_stepping)。
