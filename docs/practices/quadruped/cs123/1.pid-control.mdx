...
*$K_p=20,\ K_d=1$，稳态略偏 $q_d$，差值就是 [§1.6.4](/docs/practices/quadruped/cs123/pid-control#164-实验扰动误差) 的稳态误差。*

</div>

### 1.6.3 实验：增益响应

只看 [§1.6.2](/docs/practices/quadruped/cs123/pid-control#162-单摆-pd) 里 $K_p=20,\ K_d=1$ 那条单次响应，$K_p$、$K_d$ 仍然像两个调到不报错就行的旋钮。这一节固定 $K_p$，让 $K_d$ 扫过欠阻尼 / 接近临界 / 过阻尼三个区段，把每个旋钮单独负责什么看清楚。
...
1. **$K_p$ 决定回正力矩的量级**。$K_p$ 要先大到能抵消重力矩 $\tau_g = m g L/2 \cdot \sin q_d$，单摆才能到达 $q_d$ 附近；即便如此稳态仍会偏一截，这个偏差正是 [§1.6.4](/docs/practices/quadruped/cs123/pid-control#164-实验扰动误差) 要拆解的稳态误差，调 $K_d$ 不解决。
...
到这里，我们已经能让一个单自由度关节稳定到达任意目标角度——这是后面所有内容的地基。[§5](/docs/practices/quadruped/cs123/gait-control) 会把同一套 PD 同时套到四足机器人的全部关节上去拼出 trot 步态；再之后的 RL 步态、LLM 控制，本质上都在找一个更复杂的 $\pi(q,\dot q,\dots)\to \tau$，而最内层那条 PD 通常仍然存在。
...
3. [§1.5.2](/docs/practices/quadruped/cs123/pid-control#152-弹簧-阻尼器) 把 $K_d$ 称为"阻尼系数"，[§1.6.3](/docs/practices/quadruped/cs123/pid-control#163-实验增益响应) 又强调"$K_d$ 不是 MJCF 里的 `damping`"。这两个"阻尼"在物理上有什么区别？如果只在 MJCF 里把 `damping` 调大，能不能替代控制器里的 $K_d$？
...
到这里我们已经能让一个单关节 PD 稳定到达任意目标。组队 Lab 1 把同一个 PD 接到 Pupper HFE 关节上**换到频域看一遍**——用一系列正弦输入扫频，在 Bode 图上读出 [§1.5](/docs/practices/quadruped/cs123/pid-control#15-pd-控制) 那对 $\omega_n, \zeta$，再亲眼看"力矩饱和把高频压平"是怎么发生的。

<div align="center">

![Lab 1 teaser：HFE 跟踪 0.3 Hz 正弦](figs/lab1_hfe_sine.gif)

*HFE 关节跟踪 0.3 Hz 正弦目标：左侧是 Pupper 单腿响应，右侧是同一组数据实时打到 Bode 图上的两个点（一组幅值、一组相位）。低频 $|G|\approx 1$、相位 $\approx 0$；扫到高频段我们会看到增益下降、相位往 $-180°$ 走——本章 [§1.5](/docs/practices/quadruped/cs123/pid-control#15-pd-控制) 那对 $\omega_n, \zeta$ 在频域留下的指纹。*
...
- [ ] 找 $-3$ dB 带宽——高频段曲线偏离一阶低通形状的部分，对应 [§1.6.3](/docs/practices/quadruped/cs123/pid-control#163-实验增益响应) 里的哪条坑？

完整 starter / 测试 / 交付清单见 [`exercises/lab_1_pid_bode/`](https://github.com/datawhalechina/dive-into-embodied-ai/tree/master/codes/practices/quadruped/cs123/exercises/lab_1_pid_bode)。
