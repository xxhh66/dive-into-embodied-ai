...
3. 1.5.2 节把 $K_d$ 称为"阻尼系数"，1.6.3 节又强调"$K_d$ 不是 MJCF 里的 `damping`"。这两个"阻尼"在物理上有什么区别？如果只在 MJCF 里把 `damping` 调大，能不能替代控制器里的 $K_d$？
...
- [ ] 找 $-3$ dB 带宽——高频段曲线偏离一阶低通形状的部分，对应 1.6.3 里的哪条坑？
